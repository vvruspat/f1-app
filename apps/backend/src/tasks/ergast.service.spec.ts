...
describe("ErgastService", () => {
	...
	describe("syncSeasons", () => {
		it("returns seasons from db if present and current", async () => {
			const currentYear = new Date().getFullYear().toString();
			const seasons = [{ season: currentYear }];
			seasonModel.find().lean.mockResolvedValue(seasons);

			const result = await service.syncSeasons();
			expect(result).toEqual(seasons);
		});

		it("fetches and saves seasons if db is empty", async () => {
			const currentYear = new Date().getFullYear().toString();
			seasonModel.find().lean.mockResolvedValue([]);
			const apiSeasons = [{ season: currentYear, url: "url" }];
			jest
				.spyOn<ErgastService, "fetchSeasons">(service, "fetchSeasons")
				.mockResolvedValue({
					MRData: {
			...
			const result = await service.syncSeasons();
			expect(result).toEqual(apiSeasons);
	...
	describe("syncSeasonsResults", () => {
		it("fetches and saves missing previous season results", async () => {
			const currentYear = new Date().getFullYear().toString();
			const seasons = [{ season: "2000" }, { season: currentYear }];
			seasonModel.find().lean.mockResolvedValue(seasons);
			...
			jest
				.spyOn<ErgastService, "fetchSeasonResults">(
					service,
					"fetchSeasonResults",
				)
			...
			await service.syncSeasonsResults();

			expect(resultsModel.create).toHaveBeenCalledWith({
				season: "2000",
				Races: races,
			});
			expect(resultsModel.findOneAndUpdate).toHaveBeenCalledWith(
				{ season: currentYear },
				{ season: currentYear, Races: races },
